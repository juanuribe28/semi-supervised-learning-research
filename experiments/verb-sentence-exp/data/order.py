...
DataDict = Dict[str, Set[Tuple[str]]]
    ...
    with open(data_path) as lines:
        for line in lines:
            ...
            if label in data_dict:
                data_dict[label].add((sentence, verb))
            else:
                data_dict[label] = set([(sentence, verb)])
    return data_dict
...
def save_tsv_data(datasets: List[DataDict], file_names: List[str], exp_dir: str = '') -> None:
    for dataset, file_name in zip(datasets, file_names):
        with open('./{}{}_data.tsv'.format(exp_dir, file_name), 'w', newline='', encoding='utf8') as out_file:
            tsv_writer = csv.writer(out_file, delimiter='\t')
            for label, sent_set in dataset.items():
                for sentence, verb in sent_set:
                    tsv_writer.writerow([sentence, verb, label])
    return
...
if __name__ == "__main__":
    ...
    all_data_path = './{}all_data.tsv'.format(exp_dir)

    all_data = update_examples_dict(all_data_path)
    ...
    save_tsv_data([all_data, train_data, test_data], ['new_all', 'train', 'test'], exp_dir=exp_dir)
